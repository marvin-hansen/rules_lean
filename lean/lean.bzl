...
def _dep_manifest_lines(dep_files, dep_marker_dirs, consumer_tops):
    """Build the topo-compile manifest lines for a target's dependencies.

    Oleans sharing one of the consumer's top-level namespaces are `stage`d into
    the compile root, because Lean will not fall through LEAN_PATH roots for a
    namespace. Disjoint deps go on `leanpath` instead, with no copy.
    """
    lines = []
    leanpath = {}
...
def lean_regen_test(name, srcs, entry, expected, out = None, deps = None, data = None, tags = None):
    ...
    Args:
      ...
      data: optional runtime files the entry point reads while emitting
        (passed through to `lean_emit`).
