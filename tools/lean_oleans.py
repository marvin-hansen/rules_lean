...
#
# Invoked by a consumer's repin step. Two jobs:
#
...
def main() -> int:
    ...
    # Where the consumer's own proofs live; their imports define the closure.
    proofs_dir = sys.argv[3] if len(sys.argv) > 3 else "lean"

    index = module_index(packages_dir)
    tracked = subprocess.run(
        ["git", "ls-files", "--", proofs_dir], capture_output=True, text=True
    ).stdout.split()
    tracked = [f for f in tracked if f.endswith(".lean")]
